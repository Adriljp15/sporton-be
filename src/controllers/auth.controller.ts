...
const JWT_SECRET = process.env.JWT_SECRET || "SportonSecretKey1122";
...
export const initiateAdmin = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (count > 0) {
            res.status(400).json({ message: "We can only have 1 admin user, if yo want to create new admin user, please delete the user manually from the database" });
            return;
        }
